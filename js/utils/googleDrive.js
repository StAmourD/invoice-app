...
const GoogleDrive = {
  ...
  async init(clientId) {
    ...
    while (!window.google || !window.google.accounts) {
      if (Date.now() - startTime > maxWait) {
        console.warn('Google Identity Services library not loaded after 10 seconds. Google Drive features will be unavailable.');
   ...
   */
  getFolderName() {
    return this.environment === 'dev' ? this.DEV_FOLDER_NAME : this.PROD_FOLDER_NAME;
  },
  ...
  async authorize() {
    ...
    if (!window.google || !window.google.accounts) {
      throw new Error('Google Identity Services library not loaded yet. Please try again in a moment.');
    }
      ...
      try {
        ...
        this.tokenClient = google.accounts.oauth2.initTokenClient({
          ...
          callback: async (response) => {
            ...
            await SettingsStore.set('googleDriveTokenExpiry', expiryDate.toISOString());
        ...
        // Using 'consent' prompt ensures proper OAuth flow and avoids "app may not be secure" error
